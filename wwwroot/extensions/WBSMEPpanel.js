...
const WBSMEPGRID_CONFIG = {
    requiredProps: ['name','Category','VDC_TYPE_NAME','VDC_PESO','VDC_WBS','VDC_WBS_NAME','VDC_PRECO','VDC_PRIMARY_UNITS','VDC_SECUNDARY_UNITS','VDC_LENGTH','VDC_SIZE','System Type'], 
    ...
    {title: 'VDC_WBS',field: 'WBSCodigo',responsive:2},
    {title: 'VDC_WBS_NAME',field: 'WBSPartida',responsive:2},
    {title: 'VDC_TYPE_NAME',field: 'Type_Name',responsive:2},
    {title: 'VDC_PESO', field: 'PESO',responsive:2,topCalc:"sum", topCalcParams:{precision:1,}},
    ...
    createRow: (dbid, name, props) => { // Function generating grid rows based on recieved object properties
        ...
        const WBSCodigo = props.find(p => p.displayName === 'VDC_WBS')?.displayValue;
        const WBSPartida = props.find(p => p.displayName === 'VDC_WBS_NAME')?.displayValue;
        ...
        return {dbid, name,units,s_units,category,vdcsize,length,preco,WBSCodigo,WBSPartida,weight,Type_Name,SystemType};
    },
...
export class WBSMEPpanel extends Autodesk.Viewing.UI.DockingPanel{
    ...
    initialize(){
        ...
        this.content.innerHTML = `<div class="wbs-mep-container" style="position: relative; height: 350px;"></div>`;
        ...
        this.table = new Tabulator('.wbs-mep-container', {
            ...
            columns: WBSMEPGRID_CONFIG.columns,
            groupBy: WBSMEPGRID_CONFIG.groupBy,
            selectable:true,
            rowClick: (e, row) => WBSMEPGRID_CONFIG.onRowClick(row.getData(), this.extension.viewer)
        });
    ...
    update(model, dbids) {
        model.getBulkProperties(dbids, { propFilter: WBSMEPGRID_CONFIG.requiredProps }, (results) => {
            this.table.replaceData(results.map((result) => WBSMEPGRID_CONFIG.createRow(result.dbId, result.name, result.properties)));
        }, (err) => {
            console.error(err);
        });
