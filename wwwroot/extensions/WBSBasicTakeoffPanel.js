const WBSTAKEOFFGRID_CONFIG = {
    requiredProps: ['name','Category','VDC_WBS','VDC_PRECO','VDC_PRIMARY_UNITS','VDC_SECUNDARY_UNITS','VDC_TYPE_NAME','VDC_LENGTH','VDC_AREA','VDC_VOLUME','VDC_WBS_NAME'], // Which properties should be requested for each object
    ...
    columns: [ // Definition of individual grid columns (see http://tabulator.info for more details)
        ...
        {title: 'VDC_WBS',field: 'CodigoWBS',responsive:2},
        {title: 'VDC_WBS_NAME',field: 'NombreWBS',responsive:2},
    ...
    createRow: (dbid, name, props) => { // Function generating grid rows based on recieved object properties
        ...
        const CodigoWBS = props.find(p => p.displayName === 'VDC_WBS')?.displayValue;
        const NombreWBS = props.find(a => a.displayName === 'VDC_WBS_NAME')?.displayValue;
        ...
        return { dbid, name,CodigoWBS,NombreWBS ,preco, units,s_units, Type_Name,LENGTH, AREA, VOLUME,category};
    },
...
export class WBSBasicTakeoffPanel extends Autodesk.Viewing.UI.DockingPanel{
    ...
    initialize(){
        ...
        this.content.innerHTML = `<div class="wbstakeoff-container" style="position: relative; height: 350px;"></div>`;
        ...
        this.table = new Tabulator('.wbstakeoff-container', {
            ...
            columns: WBSTAKEOFFGRID_CONFIG.columns,
            groupBy: WBSTAKEOFFGRID_CONFIG.groupBy,
            selectable:true,
            rowClick: (e, row) => WBSTAKEOFFGRID_CONFIG.onRowClick(row.getData(), this.extension.viewer)
        });
    ...
    update(model, dbids) {
        model.getBulkProperties(dbids, { propFilter: WBSTAKEOFFGRID_CONFIG.requiredProps }, (results) => {
            this.table.replaceData(results.map((result) => WBSTAKEOFFGRID_CONFIG.createRow(result.dbId, result.name, result.properties)));
        }, (err) => {
            console.error(err);
        });
