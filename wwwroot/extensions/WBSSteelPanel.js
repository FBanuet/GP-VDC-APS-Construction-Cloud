const WBSSTEEL_CONFIG = {
    requiredProps: ['name','Category','VDC_M/PUL','VDC_PRECO','VDC_WBS','VDC_IMCA','VDC_AISC','VDC_LENGTH','VDC_WEIGHT','VDC_WBS_NAME'], // Which properties should be requested for each object
    ...
    columns: [ // Definition of individual grid columns (see http://tabulator.info for more details)
        ...
        {title: 'VDC_WBS', field: 'CodigoWBS', width: 180,responsive:0 },
        {title: 'VDC_WBS_NAME',field: 'WBSPartida',responsive:0 ,topCalc:"sum", topCalcParams:{precision:1,}},
    ...
    createRow: (dbid, name, props) => { // Function generating grid rows based on recieved object properties
        ...
        const CodigoWBS = props.find(x => x.displayName === 'VDC_WBS')?.displayValue;
        const WBSPartida = props.find(x => x.displayName === 'VDC_WBS_NAME')?.displayValue;
        return { dbid, name,MassDensity,preco,imca,section,Length,weight,category,CodigoWBS,WBSPartida};
    },
...
export class WBSSteelPanel extends Autodesk.Viewing.UI.DockingPanel {
    ...
    initialize() {
        ...
        this.content.innerHTML = `<div class="wbssteel-container" style="position: relative; height: 350px;"></div>`;
        ...
        this.table = new Tabulator('.wbssteel-container', {
            ...
            columns: WBSSTEEL_CONFIG.columns,
            groupBy: WBSSTEEL_CONFIG.groupBy,
            rowClick: (e, row) => WBSSTEEL_CONFIG.onRowClick(row.getData(), this.extension.viewer)
        });
    ...
    update(model, dbids) {
        model.getBulkProperties(dbids, { propFilter: WBSSTEEL_CONFIG.requiredProps }, (results) => {
            this.table.replaceData(results.map((result) => WBSSTEEL_CONFIG.createRow(result.dbId, result.name, result.properties)));
        }, (err) => {
            console.error(err);
        });
    }
